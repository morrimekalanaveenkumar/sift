...
type Doc = { file: string; kind: string; vendor: string; truth: Truth; notes: string[] };
...
async function invoice(vendor: Vendor, n: number, opts: { rotate?: boolean } = {}) {
  ...
  await writeFile(join(OUT, file), await pdf.save());
  docs.push({
...
async function main() {
  ...
  await writeFile(join(OUT, 'ground-truth.json'), JSON.stringify(docs, null, 2));

  const byKind = docs.reduce<Record<string, number>>((a, d) => ({ ...a, [d.kind]: (a[d.kind] ?? 0) + 1 }), {});
  console.log(`Wrote ${docs.length} documents to corpus/`);
  for (const [k, v] of Object.entries(byKind)) console.log(`  ${k}: ${v}`);
  console.log(`  ${VENDORS.length} vendors, each naming the same fields differently`);
}
